...
function! s:RookieGitDoFetch(timer) abort
    ...
    if exists('*jobstart')
        call jobstart(['git', '-C', getcwd(), 'fetch', '--all', '--quiet'], {'detach': v:true})
    elseif exists('*job_start')
        call job_start(['git', '-C', getcwd(), 'fetch', '--all', '--quiet'], {
            \ 'in_io': 'null',
            \ 'out_io': 'null',
            \ 'err_io': 'null'
        \ })
        ...
        if has('win32') || has('win64')
            call system('cmd /c start "" /B git -C ' . shellescape(getcwd()) . ' fetch --all --quiet')
        else
            call system('sh -c ' . shellescape('git -C ' . getcwd() . ' fetch --all --quiet >/dev/null 2>&1 &'))
        endif
    endif
endfunction
